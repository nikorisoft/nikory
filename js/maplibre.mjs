...
class NitroxController {
    ...
    activateLine(id) {
        ...
        if (t != null) {
            this.map.fitBounds(
                [
                    [Math.min(t.lon1, t.lon2), Math.min(t.lat1, t.lat2)],
                    [Math.max(t.lon1, t.lon2), Math.max(t.lat1, t.lat2)],
                ],
                {
     ...
     */
    constructor(containerId, transportations, visits) {
...
export function initMap(containerId) {
    const { transportations, visits } = rebuildNitroxData();

    const controller = new NitroxController(containerId, transportations, visits);
}
